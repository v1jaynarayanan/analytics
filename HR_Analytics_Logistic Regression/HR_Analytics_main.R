...
setwd("~/Downloads/Demo/HR_Analytics")
...
groupByAndSummarise(general_data, quo(Attrition))
...
# Gain, KS Statistic, ROC and Lift
...
# Gain Chart
plot(test_performance_values, main="Gain Chart")

# KS Statistic
ks_table <- attr(test_performance_values, "y.values")[[1]] - 
  (attr(test_performance_values, "x.values")[[1]])

max(ks_table)
# KS Statistic is above 40% and it shows that the model discriminates the two classes well.

# Lift chart
lift_perf <- performance(test_pred,"lift","rpp")
plot(lift_perf, main="Lift Curve")
